...
export async function POST(request: Request) {
  ...
  try {
    ...
    const target = resolveQueueChairTarget("SKIP_SPEAKER", roomState, countryId);
    ...
    const log = await prisma.chairActionLog.create({
      data: {
        ...
        threadId,
        note: buildQueueChairActionNote("SKIP_SPEAKER", country.name, target),
        metadata: buildQueueChairActionMetadata("SKIP_SPEAKER", chairAssignment, target),
      },
      select: {
        id: true,
        type: true,
