...
export async function POST(request: Request) {
  let body: QueueNudgeBody;

  try {
    body = (await request.json()) as QueueNudgeBody;
  } catch {
  ...
  try {
    ...
    const target = resolveQueueChairTarget("NUDGE_SPEAKER", roomState, countryId);
    ...
    if (!response.ok) {
      return NextResponse.json({ error: "Unable to nudge speaker" }, { status: response.status });
    }
    ...
    const log = await prisma.chairActionLog.create({
      data: {
        ...
        threadId,
        note: buildQueueChairActionNote("NUDGE_SPEAKER", country.name, target),
        metadata: buildQueueChairActionMetadata("NUDGE_SPEAKER", chairAssignment, target),
      },
      select: {
        id: true,
        type: true,
    ...
    console.error("Failed to nudge speaker.", error);
    return NextResponse.json({ error: "Unable to nudge speaker" }, { status: 500 });
  }
}
