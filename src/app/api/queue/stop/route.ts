...
export async function POST(request: Request) {
  let body: QueueStopBody;

  try {
    body = (await request.json()) as QueueStopBody;
  } catch {
  ...
  try {
    ...
    const target = resolveQueueChairTarget("STOP_SPEAKER", roomState, countryId);
    ...
    if (!response.ok) {
      return NextResponse.json({ error: "Unable to stop speaker" }, { status: response.status });
    }
    ...
    const log = await prisma.chairActionLog.create({
      data: {
        ...
        threadId,
        note: buildQueueChairActionNote("STOP_SPEAKER", country.name, target),
        metadata: buildQueueChairActionMetadata("STOP_SPEAKER", chairAssignment, target),
      },
      select: {
        id: true,
        type: true,
    ...
    return NextResponse.json({ error: "Unable to stop speaker" }, { status: 500 });
  }
}
